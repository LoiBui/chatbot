...
class Page(TenantAppHelper):
  ...
  def processOfRequest(self, tenant):
		try:
			...
			self.render('template_index.html', self._design_type, template_vals)
		except BaseException, e:
			self.outputErrorLog(e)
			self.redirectError(UcfMessage.getMessage(self.getMsg('MSG_SYSTEM_ERROR'), ()))
			return

app = ndb.toplevel(webapp2.WSGIApplication([('/a/([^/]*)/template', Page)], debug=sateraito_inc.debug_mode, config=sateraito_func.wsgi_config))