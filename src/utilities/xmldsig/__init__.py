...
PTN_SIGNED_INFO_XML_WITH_EXCLUSIVE_C14N = \
...
PTN_SIGNED_INFO_XML_WITH_EXCLUSIVE_C14N_WITHCOMMENTS = \
...
def sign(xml, f_private, key_info_xml, key_size, with_exclusive_c14n=False, with_comments=False, reference_uri='', after_issuer=False, sig_id_value=None):
  ...
  """
  logging.info('***** unsigned_xml *****')
  logging.info(xml)
  logging.info('***** reference_uri *****')
  logging.info(reference_uri)
  logging.info('***** with_exclusive_c14n *****')
  logging.info(with_exclusive_c14n)
  logging.info('***** with_comments *****')
  logging.info(with_comments)
...
def verify(xml, f_public, key_size, with_exclusive_c14n=False, reference_uri=''):
  ...
  signed_info_xml = _signed_info(unsigned_xml, with_exclusive_c14n=with_exclusive_c14n, reference_uri=reference_uri)
...
def _signed_info(xml, with_exclusive_c14n=False, with_comments=False, reference_uri=''):
  ...
  if with_exclusive_c14n:
    # WithCommentsに対応していないSPもあるようなので（mellonなど）... 2016.08.05
    if with_comments:
      ptn_signed_info_xml = PTN_SIGNED_INFO_XML_WITH_EXCLUSIVE_C14N_WITHCOMMENTS
    else:
      ptn_signed_info_xml = PTN_SIGNED_INFO_XML_WITH_EXCLUSIVE_C14N
