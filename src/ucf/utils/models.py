...
## 他の設定
############################################################
class FAQMoreConfig(UCFModel2):
	unique_id = ndb.StringProperty(required=True)
	ip_address = ndb.StringProperty(repeated=True)
	# sort_order = ndb.StringProperty(default='priority')
	created_date = ndb.DateTimeProperty(auto_now_add=True)
	updated_date = ndb.DateTimeProperty(auto_now=True)
	
	@classmethod
	def createMoreConfig(cls, ip_address):
	...
	@classmethod
	def updateMoreConfig(cls, unique_id, ip_address=None):
		if ip_address is None:
			ip_address = []
		entry = cls.query()
		entry = entry.filter(cls.unique_id == unique_id)
		entry = entry.get()
		if entry is None:
			entry = cls(unique_id=unique_id, id=unique_id)
		entry.ip_address = ip_address
...
## File Up Platform 設定
############################################################
class FileUpSettingConfig(UCFModel2):
	unique_id = ndb.StringProperty(required=True)
	channel_config = ndb.TextProperty()
	channel_kind = ndb.StringProperty()
	folder_node = ndb.StringProperty()
	created_date = ndb.DateTimeProperty(auto_now_add=True)
	updated_date = ndb.DateTimeProperty(auto_now=True)
	...
	def getInstance(cls, unique_id):
		...
		row = q.get(keys_only=True)
		result = row.get() if row is not None else None
		return result
	
	@classmethod
	def getSettingConfigByChannelKind(cls, channel_kind):
		q = cls.query()
		q = q.filter(cls.channel_kind == channel_kind)
		row = q.get(keys_only=True)
		result = row.get() if row is not None else None
		return result
	
	@classmethod
	def modifyFileUpSettingConfig(cls, unique_id, channel_config, channel_kind):
		entry = cls.query()
		entry = entry.filter(cls.unique_id == unique_id)
		entry = entry.get()
		if entry is None:
		...
		entry.channel_config = channel_config
		entry.channel_kind = channel_kind
		entry.put()
	
	@classmethod
	def getRichMenuId(cls, channel_kind):
		q = cls.query()
		q = q.filter(cls.channel_kind == channel_kind)
		row = q.get()
		if row is not None:
			channel_config = json.JSONDecoder().decode(row.channel_config)
			if channel_config and 'richmenu_id' in channel_config:
				return channel_config['richmenu_id']
		return None
	
	@classmethod
	def getFolderNode(cls, channel_kind):
		q = cls.query()
		q = q.filter(cls.channel_kind == channel_kind)
		row = q.get()
		if row is not None:
			if row.folder_node:
				return row.folder_node
		return ''
	
	@classmethod
	def setFolderNode(cls, channel_kind, node):
		q = cls.query()
		q = q.filter(cls.channel_kind == channel_kind)
		entry = q.get()
		if entry is not None:
			entry.folder_node = node
			entry.put()
...
## Direct Cloud Box 設定
############################################################
class DirectCloudBoxSettingConfig(UCFModel2):
	unique_id = ndb.StringProperty(required=True)
	code = ndb.StringProperty()
	uid = ndb.StringProperty()
	password = ndb.StringProperty()
	service = ndb.StringProperty()
	service_key = ndb.StringProperty()
	user_seq = ndb.IntegerProperty()
	created_date = ndb.DateTimeProperty(auto_now_add=True)
	updated_date = ndb.DateTimeProperty(auto_now=True)
	...
	@classmethod
	def modifyConfig(cls, unique_id, code, uid, password, service, service_key, user_seq):
		entry = cls.query()
		entry = entry.filter(cls.unique_id == unique_id)
		entry = entry.get()
		if entry is None:
...
## File Server 設定
############################################################
class FileServerSettingConfig(UCFModel2):
	unique_id = ndb.StringProperty(required=True)
	file_server_platform = ndb.StringProperty()
	file_server_config = ndb.TextProperty()
	is_saved = ndb.BooleanProperty()
	created_date = ndb.DateTimeProperty(auto_now_add=True)
	updated_date = ndb.DateTimeProperty(auto_now=True)
	
	@classmethod
	def getConfig(cls, platform):
		q = cls.query()
		q = q.filter(cls.file_server_platform == platform)
		row = q.get(keys_only=True)
		result = row.get() if row is not None else None
		return result
	
	@classmethod
	def getFileServerConfig(cls, platform):
		q = cls.query()
		q = q.filter(cls.file_server_platform == platform)
		row = q.get()
		if row is not None:
			return json.JSONDecoder().decode(row.file_server_config)
		return None
	
	@classmethod
	def modifyConfig(cls, unique_id, file_server_config, file_server_platform, is_saved):
		entry = cls.query()
		entry = entry.filter(cls.unique_id == unique_id)
		entry = entry.get()
		if entry is None:
			entry = cls(unique_id=unique_id, id=unique_id)
			entry.unique_id = unique_id
		entry.file_server_config = file_server_config
		entry.file_server_platform = file_server_platform
		entry.is_saved = is_saved
		entry.put()
	
	@classmethod
	def getDCBUserSeq(cls, platform):
		q = cls.query()
		q = q.filter(cls.file_server_platform == platform)
		row = q.get()
		if row is not None:
			if row.file_server_config:
				file_server_config = json.JSONDecoder().decode(row.file_server_config)
				if 'user_seq' in file_server_config:
					return file_server_config['user_seq']
		return None


class FileStorage(UCFModel2):
	unique_id = ndb.StringProperty(required=True)
	file_key = ndb.StringProperty()
	blob_key = ndb.StringProperty()
	storage_platform = ndb.StringProperty()
	created_date = ndb.DateTimeProperty(auto_now_add=True)
	updated_date = ndb.DateTimeProperty(auto_now=True)
	
	@classmethod
	def saveFile(cls, file_key, blob_key, storage_platform):
		unique_id = UcfUtil.guid()
		entry = cls(unique_id=unique_id, id=unique_id)
		entry.file_key = file_key
		entry.blob_key = blob_key
		entry.storage_platform = storage_platform
		entry.put()
	
	@classmethod
	def getBlobKey(cls, file_key, storage_platform):
		q = cls.query()
		q = q.filter(cls.file_key == file_key)
		q = q.filter(cls.storage_platform == storage_platform)
		row = q.get()
		if row is not None:
			return row.blob_key
		return None
