# coding: utf-8
#######################################################################
# ucf.sessions:gaesessions‚ðŒ³‚É‰ÁH
# sid‚ð’Z‚­‚·‚é‚È‚Ç
#######################################################################
...
#SIG_LEN = 0	# base 64 encoded HMAC-SHA256
MAX_COOKIE_LEN = 4096
...
#				# to md5 by T.ASAO because too long.
 #			 mdl = hashlib.md5()
	#			mdl.update(cv)
	 #		 cv = mdl.hexdigest()
...
def delete_expired_sessions():
		...
		try:
			...
			key_str = now_str + u'\ufffd'
			key = db.Key.from_path('SessionModel', key_str, namespace='')
			q.filter('__key__ < ', key)
			results = q.fetch(500)
			db.delete(results)
			logging.info('key_str=' + key_str + ' target_cnt=' + str(len(results)))
			namespace_manager.set_namespace(strOldNamespace)
			return len(results) < 500
		except Exception, e:
			namespace_manager.set_namespace(strOldNamespace)
			logging.error(str(e))
			return True	# ƒGƒ‰[‚Ìê‡‚ÍI—¹‚µ‚½‚¢‚Ì‚ÅTrue
