...
import webapp2,logging,datetime,time,gc
...
class LoginHistoryUtils():
	...
	#	try:
	#		vo['login_password'] = helper.encryptoData(vo['login_password'], enctype='AES')		# パスワード暗号化
	#		vo['login_password_enctype'] = 'AES'
	#	except UnicodeEncodeError, e:
	#		vo['login_password'] = helper.encryptoData(UcfUtil.urlEncode(vo['login_password']), enctype='AES')		# パスワード暗号化（全角とかの場合はエラーしちゃうのでURLエンコードしてから暗号化）
	#		vo['login_password_enctype'] = 'AES'
	...
	def createCsv(cls, helper, login_operator_entry=None, sk_operator_unique_id='', optional_scond=None):
		...
		while True:
			...
			for entry in each_rows:
				...
				data.append(UcfUtil.getHashStr(vo, 'client_x_forwarded_for_ip'))					# ipaddress_xff
	...
	def getCsvTitles(cls, helper):
		...
		return ['command','access_date','login_id','login_password_length','email','login_type','login_result','ipaddress','ipaddress_xff','use_profile_id','target_career','target_env','is_auto_login','user_agent','log_code']
