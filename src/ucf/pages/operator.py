...
class OperatorUtils():
  ...
  # 取得用：データ加工
  def editVoForSelect(cls, helper, vo, is_with_parent_group_info=False):
    ...
    # 親グループ情報（ユーザテーブルに保持しなくなったので）
    if is_with_parent_group_info:
      ParentGroupInfo = []
      if UcfUtil.getHashStr(vo, 'operator_id_lower') != '':
        #gql = ''
        #wheres = []
        #wheres.append("belong_members IN ('" + UcfUtil.escapeGql(UcfUtil.getHashStr(vo, 'operator_id_lower')) + "')")
        #gql += UcfUtil.getToGqlWhereQuery(wheres)
        #entrys = UCFMDLOperatorGroup.gql(gql)
        q = UCFMDLOperatorGroup.query()
        q = q.filter(UCFMDLOperatorGroup.belong_members == UcfUtil.getHashStr(vo, 'operator_id_lower'))
  ...
  def editVoForRegist(cls, helper, vo, entry_vo, edit_type, is_noupdate_password_change_date_for_sync=False):
    ...
    #	vo['operator_id'] = UcfUtil.getHashStr(vo, 'operator_id_localpart') + '@' + UcfUtil.getHashStr(vo, 'federated_domain')		# メールアドレス
    vo['operator_id_lower'] = UcfUtil.getHashStr(vo, 'operator_id').lower()									# 小文字（検索、重複チェック用）
    #vo['employee_id_lower'] = UcfUtil.getHashStr(vo, 'employee_id').lower()									# 小文字（検索、重複チェック用）
  ...
  # リクエストから親グループ情報を取得
  def getParentGroupInfoFromRequest(cls, vo):
    parent_group_info = None
    if UcfUtil.getHashStr(vo, 'ParentGroupInfo') != '':
      parent_group_info = JSONDecoder().decode(UcfUtil.getHashStr(vo, 'ParentGroupInfo'))
    return parent_group_info
  getParentGroupInfoFromRequest = classmethod(getParentGroupInfoFromRequest)
...
#	# 社員IDからユーザVoを取得
#	def getUserByEmployeeID(cls, helper, employee_id):
#		user_vo = None
#		if employee_id and employee_id != '':
#			query = UCFMDLOperator.query()
#			query = query.filter(UCFMDLOperator.employee_id_lower == employee_id.lower())
#			key = query.get(keys_only=True)
#			entry = key.get() if key is not None else None
#			if entry is not None:
#				user_vo = entry.exchangeVo(helper._timezone)
#				OperatorUtils.editVoForSelect(helper, user_vo)
#		return user_vo
#	getUserByEmployeeID = classmethod(getUserByEmployeeID)

  # クライアント証明書のコモンネームからユーザーを取得
  def getUserByClientCertificateCN(cls, helper, client_certificate_cn):
    user_vo = None
    if client_certificate_cn and client_certificate_cn.strip() != '':
      query = UCFMDLOperator.query()
      query = query.filter(UCFMDLOperator.client_certificate_cn_lower == client_certificate_cn.strip().lower())
      key = query.get(keys_only=True)
      entry = key.get() if key is not None else None
      if entry is not None:
        user_vo = entry.exchangeVo(helper._timezone)
        OperatorUtils.editVoForSelect(helper, user_vo)
    return user_vo
  getUserByClientCertificateCN = classmethod(getUserByClientCertificateCN)

  # このグループをメイン組織に設定しているユーザのメイン組織をクリア
  def removeUsersTargetMainGroup(cls, helper, group_id, operator_id=''):
    if group_id != '':
      group_id = group_id.lower()
      query = UCFMDLOperator.query()
      query = query.filter(UCFMDLOperator.main_group_id == group_id)
      for entry in query:
        # メイン組織IDをクリア
...
#				entry.margeFromVo(vo, helper._timezone)
        ...
        # 更新処理
        entry.put()
  removeUsersTargetMainGroup = classmethod(removeUsersTargetMainGroup)

  def getOperatorMappingClass(cls,entry):
    result = []
    mergeList = sateraito_inc.MERGE_FIELD_CLASS_OPERATOR
    if entry is None:
      return result
    ...
    for item in mergeList:
      for field in  item['keys']:
        if user_dict.has_key(field):
          ...
          field_format = sateraito_inc.MAIL_MERGE_FORMAT.format(field)
          item['content'] = content.replace(field_format,user_dict[field])
      result.append(item)
    return result
  getOperatorMappingClass = classmethod(getOperatorMappingClass)
  ...
  def margeVoFromCsvRecord(cls, helper, vo, csv_record, login_operator_entry):
    ...
    if csv_record.has_key('language'):
    ...
    if csv_record.has_key('account_stop'):
      vo['account_stop_flag'] = csv_record['account_stop'].strip()
    if csv_record.has_key('password_change'):
      vo['next_password_change_flag'] = csv_record['password_change'].strip()
    if csv_record.has_key('password_expire'):
      vo['password_expire'] = csv_record['password_expire'].strip()
    if csv_record.has_key('login_lock'):
      vo['login_lock_flag'] = csv_record['login_lock'].strip()
    if csv_record.has_key('login_lock_expire'):
      vo['login_lock_expire'] = csv_record['login_lock_expire'].strip()
    if csv_record.has_key('password') and csv_record.has_key('password_update_flag') and csv_record['password_update_flag'] == 'UPDATE':
  ...
  def getCsvTitles(cls, helper):
    ...
    return ['command','user_id','email','last_name','first_name','last_name_kana','first_name_kana','sub_email','access_authority','delegate_function','delegate_management_groups','language','management_group','account_stop','password_change','password_expire','password_change_date','login_lock','login_lock_expire','login_count','login_failed_count','last_login_date','last_login_password_length','password','password_update_flag','company','company_office','company_department','company_department2','job_title','email_work','email_work_phone','phone_work','fax_work','mobile_phone','extension_number','pocketbell','postal_country','postal_code','postal_prefecture','postal_city','postal_street_address','comment']
  ...
  def changeOneOperatorID(cls, helper, src_email, dst_email, entry, is_operator, delegate_management_groups, record_cnt=0, is_direct_taskprocess=False, login_operator_id=''):
    ...
    OperatorUtils.editVoForSelect(helper, entry_vo, is_with_parent_group_info=False)		# データ加工（取得用）
    ...
    code, msg = TaskChangeIDUtils.insertTask(helper, 'change_operator_id', entry.unique_id, src_email, dst_email, sync_result, queue_name='tenant-execute-task', is_direct_taskprocess=is_direct_taskprocess)
...
#			self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 80)))
    ...
    # 役職
    check_name = helper.getMsg('FLD_CONTACT_COMPANY_POST')
    check_key = 'contact_company_post'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：64文字（O365に合わせて）
    ...
    # 部署
    check_name = helper.getMsg('FLD_CONTACT_COMPANY_DEPARTMENT')
    check_key = 'contact_company_department'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：64文字（O365に合わせて）
    if not self.maxLengthValidator(check_value, 64):
      self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 64)))
    ...
    # 事業所
    check_name = helper.getMsg('FLD_CONTACT_COMPANY_OFFICE')
    check_key = 'contact_company_office'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：128文字（O365に合わせて）
    if not self.maxLengthValidator(check_value, 128):
      self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 128)))

    # 会社電話
    check_name = helper.getMsg('FLD_CONTACT_TEL_NO1')
    ...
    # FAX
    check_name = helper.getMsg('FLD_CONTACT_TEL_NO2')
    ...
    # 携帯電話
    check_name = helper.getMsg('FLD_CONTACT_TEL_NO3')
    ...
    # 番地
    check_name = helper.getMsg('FLD_CONTACT_POSTAL_STREET_ADDRESS')
    check_key = 'contact_postal_street_address'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：1024文字（O365に合わせて）
    if not self.maxLengthValidator(check_value, 1024):
      self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 1024)))

    # 市区町村
    check_name = helper.getMsg('FLD_CONTACT_POSTAL_CITY')
    check_key = 'contact_postal_city'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：128文字（O365に合わせて）
    if not self.maxLengthValidator(check_value, 128):
      self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 128)))
    ...
    # 都道府県
    check_name = helper.getMsg('FLD_CONTACT_POSTAL_PREFECTURE')
    check_key = 'contact_postal_prefecture'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：128文字（O365に合わせて）
    if not self.maxLengthValidator(check_value, 128):
      self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 128)))
    ...
    # 郵便番号
    check_name = helper.getMsg('FLD_CONTACT_POSTAL_CODE')
    check_key = 'contact_postal_code'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：40文字（O365に合わせて）
    ...
    # 国、地域
    check_name = helper.getMsg('FLD_CONTACT_POSTAL_COUNTRY')
    check_key = 'contact_postal_country'
    check_value = UcfUtil.getHashStr(vo, check_key)
    # 最大長チェック：128文字（O365に合わせて）
    if not self.maxLengthValidator(check_value, 128):
      self.appendValidate(check_key, UcfMessage.getMessage(helper.getMsg('MSG_VC_MAXLENGTH'), (check_name, 128)))
    ...
    if self.total_count == 0:
      ...
      operator_id = ''
      #if vo.has_key('operator_id_localpart'):
      #	operator_id = UcfUtil.getHashStr(vo, 'operator_id_localpart') + '@' + UcfUtil.getHashStr(vo, 'federated_domain')
      #else:
      ...
      q = UCFMDLOperatorGroup.query()
      q = q.filter(UCFMDLOperatorGroup.group_id_lower == operator_id.lower())
      for model in q:
        # 新規以外の場合は対象のユニークＩＤ以外の場合のみエラーとする(GQLがノットイコールに対応していないため)
        if self.edit_type == UcfConfig.EDIT_TYPE_NEW or model.unique_id != unique_id:
          self.appendValidate('operator_id', UcfMessage.getMessage(helper.getMsg('MSG_VC_ALREADY_EXIST'), ()))
      ...
      ################################################
      ## 社員ＩＤ
      #if vo.get('employee_id', '') != '':
      #	gql = ''
      #	# WHERE句
      #	wheres = []
      #	wheres.append("employee_id_lower='" + UcfUtil.escapeGql(vo.get('employee_id', '').lower()) + "'")
      #	gql += UcfUtil.getToGqlWhereQuery(wheres)
      #	models = UCFMDLOperator.gql(gql)
      #	for model in models:
      #		# 新規以外の場合は対象のユニークＩＤ以外の場合のみエラーとする(GQLがノットイコールに対応していないため)
      #		if self.edit_type == UcfConfig.EDIT_TYPE_NEW or model.unique_id != unique_id:
      #			self.appendValidate('employee_id', UcfMessage.getMessage(helper.getMsg('MSG_VC_ALREADY_EXIST'), ()))
      #			break
...
class ChangeAccountIDCsvValidator(BaseValidator):
  ...
  def validate(self, helper, vo):
    ...
    if self.total_count == 0:
      q = UCFMDLOperatorGroup.query()
      q = q.filter(UCFMDLOperatorGroup.group_id_lower == new_email.lower())
      key = q.get(keys_only=True)
      if key is not None:
        self.appendValidate('new_email', UcfMessage.getMessage(helper.getMsg('MSG_VC_ALREADY_EXIST'), ()))
