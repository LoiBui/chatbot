...
import webapp2,logging,random,time
...
class TaskChangeIDUtils():
	...
	def insertTask(helper, task_type, target_unique_id, src_data_id, dst_data_id, sync_result, queue_name='default', is_direct_taskprocess=False):
		...
		if not is_direct_taskprocess:
			...
			try:
				...
				TaskChangeIDUtils.changeBelongMembersAddresses(helper, sync_result, vo, entry, is_direct_taskprocess=is_direct_taskprocess)
	...
	# （一括更新の場合はタスク処理部分も非同期ではなくインポートタスク内で直列に実施）
	def changeBelongMembersAddresses(helper, sync_result, task_vo, task_entry, is_direct_taskprocess=False):

		MAX_RETRY_CNT = 3

		execute_operator_id = sync_result.get('execute_operator_id', '')

		src_email = UcfUtil.getHashStr(task_vo, 'src_data_id').lower()
		dst_email = UcfUtil.getHashStr(task_vo, 'dst_data_id').lower()

		# 一括変更時のログに毎行出ちゃうので...
		## ログ：開始
		#sync_result['log_text'] += ucffunc.createLogRecord(helper, UcfMessage.getMessage(helper.getMsg('LOG_START_TASK')))

		################################################
		# 該当のアドレスが所属メンバーとなっているグループを処理
		MAX_PAGES = 1000
		NUM_PER_PAGE = 100
		...
		while True:
			try:
				...
				q = q.filter(UCFMDLOperatorGroup.belong_members == src_email)
				for i in range(MAX_PAGES):
					each_rows = q.fetch(limit=NUM_PER_PAGE, offset=(i * NUM_PER_PAGE))
					if len(each_rows) == 0:
						break
					for row in each_rows:
						row.belong_members.remove(src_email)
						row.belong_members.append(dst_email)
						if src_email in row.group_owners:
							row.group_owners.remove(src_email)
							row.group_owners.append(dst_email)
						row.date_changed = UcfUtil.getNow()
						row.updater_name = execute_operator_id
						row.put()
						sync_result['log_text'] += ucffunc.createLogRecord(helper, UcfMessage.getMessage(helper.getMsg('LOG_CHANGEID_FROM_GROUP_BELONG_MEMBERS'), (row.group_id, UcfUtil.getHashStr(task_vo, 'src_data_id'), UcfUtil.getHashStr(task_vo, 'dst_data_id'))))
				break
			except BaseException, instance:
				if process_cnt < MAX_RETRY_CNT:
					#logging.info(instance)
					time.sleep(1)		# すぐリトライしてもエラーすることが多いので1秒ほどスリープしてみる
				else:
	...
	changeBelongMembersAddresses = staticmethod(changeBelongMembersAddresses)


	# （一括更新の場合はタスク処理部分も非同期ではなくインポートタスク内で直列に実施）
	def changeBelongMembersAddressesOfUser(helper, sync_result, task_vo, task_entry, is_direct_taskprocess=False):

		MAX_RETRY_CNT = 3

		execute_operator_id = sync_result.get('execute_operator_id', '')

		src_user_id = UcfUtil.getHashStr(task_vo, 'src_data_id').lower()
		dst_user_id = UcfUtil.getHashStr(task_vo, 'dst_data_id').lower()
		...
		while True:
			try:
				...
				q = q.filter(sateraito_db.Group.belong_members == src_user_id)
				for i in range(MAX_PAGES):
					each_rows = q.fetch(limit=NUM_PER_PAGE, offset=(i * NUM_PER_PAGE))
					if len(each_rows) == 0:
						break
					for row in each_rows:
						row.belong_members.remove(src_user_id)
						row.belong_members.append(dst_user_id)
						if src_user_id in row.group_owners:
							row.group_owners.remove(src_user_id)
							row.group_owners.append(dst_user_id)
						row.date_changed = UcfUtil.getNow()
						row.updater_name = execute_operator_id
						row.put()
						sync_result['log_text'] += ucffunc.createLogRecord(helper, UcfMessage.getMessage(helper.getMsg('LOG_CHANGEID_FROM_GROUP_BELONG_MEMBERS'), (row.group_id, UcfUtil.getHashStr(task_vo, 'src_data_id'), UcfUtil.getHashStr(task_vo, 'dst_data_id'))))
	...
	changeBelongMembersAddressesOfUser = staticmethod(changeBelongMembersAddressesOfUser)
	...
	def _afterTaskProcess(cls, helper, task_entry, task_status, sync_result):
		if task_entry is not None:
			...
			task_entry.task_status_date = UcfUtil.getNow()
			task_entry.log_text = UcfUtil.nvl(task_entry.log_text) + unicode(UcfUtil.getHashStr(sync_result, 'log_text'))
