...
class StorageGetCommand(BaseCommandEx):
  def __init__(self, element_list, parent_command, script_row_num, params):
    super(StorageGetCommand, self).__init__(element_list, parent_command, script_row_num, params)
  ...
  def _execute(self):
    ...
    # result = bot_storage_db.BotStorage.load_storage(user_id, storage_key)
    result = bot_storage_db.BotStorageText.load_storage(user_id, storage_key)
